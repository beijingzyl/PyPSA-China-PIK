        ...
        return {
            ...
            'departments': {
                'transport': {
                    ...
                    'profile_file': '../../results/output/normalized_profiles.csv',
...
class REMINDModel(IAMModel):
    ...
    def _aggregate_transport_energy(self, df: pd.DataFrame, config: Dict) -> pd.DataFrame:
        ...
        # 如果没有ev_types_file，创建一个简单的映射
        logger.warning("未找到EV类型文件，使用默认映射")
        mapped_data_list = []
        
        # 添加所有车辆类型
        for v_type in df['Vehicle_Type'].unique():
            energy = df[df['Vehicle_Type'] == v_type]['Energy_EJ'].sum()
            if energy > 0:
                mapped_data_list.append({'mapped_type': v_type, 'Energy_EJ': energy})
